...
const StudyCreationPage = ({ onCancel, selectedBook }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log("생성 버튼을 눌렀습니다.");
        console.log(`책정보: ${JSON.stringify(selectedBook, null, 2)}`);
        console.log(`폼 데이터: ${JSON.stringify(formData, null, 2)}`);
        console.log(`리마인드: ${JSON.stringify(reminds, null, 2)}`);
        console.log(`사용자 ID: ${JSON.stringify(userIds, null, 2)}`);
        ...
        try {
            const response = await axios.post(`/api/${param.teamId}/study-pages`, studyPageDto);
            const study = response.data; // API 응답에서 study 데이터 추출

            // 리다이렉트하면서 study 상태 전달
            navigate(`/teams/${param.teamId}/study/${study.id}`, { state: { study } });
        } catch (error) {
